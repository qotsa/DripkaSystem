...
SERVICES_INFRA[activemq]="-p 8161:8161 -p 61616:61616 -p 61613:61613 --env \"ACTIVEMQ_MIN_MEMORY=512\" --env \"ACTIVEMQ_MAX_MEMORY=2048\" --env \"ACTIVEMQ_NAME=amqp-srv1\" --env \"ACTIVEMQ_REMOVE_DEFAULT_ACCOUNT=true\" --env \"ACTIVEMQ_ADMIN_LOGIN=admin\" --env \"ACTIVEMQ_ADMIN_PASSWORD=admin\" --env \"ACTIVEMQ_WRITE_LOGIN=producer\" --env \"ACTIVEMQ_WRITE_PASSWORD=producer\" --env \"ACTIVEMQ_READ_LOGIN=consumer\" --env \"ACTIVEMQ_READ_PASSWORD=consumer\" --env \"ACTIVEMQ_JMX_LOGIN=jmx\" --env \"ACTIVEMQ_JMX_PASSWORD=jmx\" --env \"ACTIVEMQ_ENABLED_SCHEDULER=true\" ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[ftp]="-p 2021:2021 -p 30000-30009:30000-30009 --mount type=volume,source=ftpData,destination=/home/ftpusers --mount type=volume,source=ftpUsers,destination=/etc/pure-ftpd/users --env \"PUBLICHOST=ftp\" ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[ftpsmev]="-p 3333:3333 -p 30010-30019:30010-30019 --mount type=volume,source=ftpSmevData,destination=/home/ftpusers --mount type=volume,source=ftpSmevUsers,destination=/etc/pure-ftpd/users --env \"PUBLICHOST=ftpsmev\" ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[postgresql]="-p 5432:5432 --mount type=volume,source=postgresData,destination=/var/lib/postgresql/data ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[elasticsearch]="-p 9200:9200 --mount type=volume,source=elasticsearchData,destination=/usr/share/elasticsearch/data ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[logstash]="-p 12201:12201 -p 4560:4560 ${REGISTRY}/cadvisor:${IMAGES_VERSION} logstash -f /config-dir/logstash.conf"
SERVICES_INFRA[curator]="${REGISTRY}/cadvisor:${IMAGES_VERSION}"
...
SERVICES_INFRA[kibana]="-p 5601:5601 --env \"--max-old-space-size=250\" ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[grafana]="-p 3000:3000 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"

declare -A SERVICES
SERVICES_INFRA[eureka]="-p 8097:8097 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[configserver]="-p 8888:8888 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[smev3core]="-p 8093:8093 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[smev3adapter]="-p 8090:8090 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[ufosadapter]="-p 8094:8094 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[replication]="-p 8095:8095 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
#SERVICES_INFRA[pollers]="-p 8092:8092 -p 5701:5701 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[pushers]="-p 8096:8096 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[smev3mock2]="-p 8091:8091 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
SERVICES_INFRA[bpmengine]="-p 8098:8098 ${REGISTRY}/cadvisor:${IMAGES_VERSION}"
